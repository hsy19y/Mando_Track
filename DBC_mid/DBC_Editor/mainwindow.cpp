...
void MainWindow::on_pushButton_clicked()
{
    ...
    g_msg_debug2 = g_msg_debug2 +QString::number(g_debug2_cnt) +"  Failed to Connect Device\n";
    ui->txt_debug_readtab_2->setText(g_msg_debug2);
    ui->txt_debug_readtab_2->verticalScrollBar()->setValue(ui->txt_debug_readtab_2->verticalScrollBar()->maximum());
}
int live_end_flag=0;
...
void MainWindow::decodeCan(QString _data){
        ...
        //for (int i = 0; i < length; ++i) {
        //    qDebug()<<"data"<<QString::number(parts[2 + i].toInt(nullptr, 16));
        //}
...
void MainWindow::on_ReadyRead()
{
    ...
    // 수신된 데이터를 특정 UI 요소에 표시하거나 처리
    ui->txt_t3->setText(QString(data));  // 예: LineEdit에 데이터 표시
}
...
{
    g_msg_debug2 = g_msg_debug2 +QString::number(g_debug2_cnt) +"  Started Recieve CAN Data\n";
    ui->txt_debug_readtab_2->setText(g_msg_debug2);
    ui->txt_debug_readtab_2->verticalScrollBar()->setValue(ui->txt_debug_readtab_2->verticalScrollBar()->maximum());
    // 새로운 신호가 선택될 때마다 테이블을 초기화합니다.
    ui->live_table->clear(); // 테이블 초기화
    //ui->live_table->setColumnCount(messages[ui->sg_select_combo_2->currentIndex()].signalList.size()); // 열 개수 설정
    ui->live_table->setColumnCount(5);
    // 헤더에 Signal의 이름을 추가합니다.
    QStringList headerLabels;
    //for (const Signal &signal : messages[ui->sg_select_combo_2->currentIndex()].signalList) {
        headerLabels << "쓰로틀 퍼센트"<<"RPM"<<"속도"<<"엔진 온도"<<"스티어 각도"; // 헤더 라벨에 Signal의 이름 추가
    //}
    ui->live_table->setHorizontalHeaderLabels(headerLabels); // 열 라벨 설정
